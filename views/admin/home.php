...
<body>
  ...
  <div class="sidebar">
    ...
    <div style="background-color: rgba(255,255,255,0.1); padding: 10px; border-radius: 8px; margin-bottom: 20px;">
      <small style="opacity: 0.8;">Connecté en tant que:</small>
  ...
  <div class="main">
    <div class="header">
      ...
      <p class="text-muted">Gestion des types de prêts</p>
    </div>
    ...
    <div class="admin-section">
      <h2 class="mb-3">📋 Gestion des Types de Prêt</h2>
      ...
      <form id="form-type-pret" onsubmit="return false;">
        <div class="form-group">
          <label for="nom_type">Nom du type de prêt :</label>
      ...
      <table class="table mt-4">
        ...
        <tbody id="table-type-pret">
          <tr>
            <td colspan="6" class="text-center text-muted">Chargement des données...</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <script>
      ...
      fetch(apiBase + "/typepret")
        ...
        .then(data => {
          ...
          data.forEach(tp => {
            ...
            const dureeAnnees = Math.round(tp.duree_mois / 12 * 10) / 10; // Arrondi à 1 décimale
    ...
    function ajouterTypePret() {
      ...
      if (parseFloat(taux) < 0 || parseFloat(taux) > 50) {
        afficherMessage('Le taux doit être entre 0 et 50%', 'error');
        return;
      }

      if (parseInt(duree) < 1 || parseInt(duree) > 600) {
        afficherMessage('La durée doit être entre 1 et 600 mois', 'error');
        return;
      }
      ...
      .then(response => {
        if (response.error) {
          ...
          afficherMessage(`Type de prêt "${nom}" ajouté avec succès!`, 'success');
      ...
      .catch(error => {
        console.error('Erreur:', error);
        afficherMessage('Erreur lors de l\'ajout du type de prêt', 'error');
      });
    }
    ...
    function supprimerTypePret(id) {
      ...
      .then(response => {
        if (response.error) {
          ...
          afficherMessage('Type de prêt supprimé avec succès', 'success');
          chargerTypesPret();
          chargerStatistiques();
        }
      })
